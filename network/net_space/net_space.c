...
int main()
{
  ...
  char* cmd;
  asprintf(&cmd, "ip link set veth11 netns %d", child_pid);
  system("ip link add veth01 type veth peer name veth11");
  system(cmd);
  system("ip link set veth01 up");
  system("ip addr add 169.254.1.1/30 dev veth01");
  free(cmd);
