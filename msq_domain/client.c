...
{
    return local_addr_send(&addr, buf, len);
}
...
int main()
{
    ...
    for(i = 0; i < 10; i++)
    {
        ...
        local_addr_send(&addr, (char *)&msg, sizeof(local_data_msg));
        sleep(1);
        ret = msg_recv(fd, 0, buf, sizeof(buf));
        if(ret > 0)
        {
            pdata = (msg_data *)buf;
